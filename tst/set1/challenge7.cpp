...
namespace {
    TEST(AESDecryptTest, ShouldDecryptChallenge) {
        const std::vector<uint8_t> KEY{'Y', 'E', 'L', 'L', 'O', 'W', ' ', 'S', 'U', 'B', 'M', 'A', 'R', 'I', 'N', 'E'};

        std::ifstream is("../res/set1/7.txt");
        std::string base64_ciphertext{std::istream_iterator<uint8_t>{is},
                                      std::istream_iterator<uint8_t>{}};

        Botan::secure_vector<uint8_t> ciphertext = Botan::base64_decode(base64_ciphertext);
        auto result = crypto::functions::aes_ecb_decrypt(KEY, {ciphertext.begin(), ciphertext.end()});
